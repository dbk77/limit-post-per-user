...
namespace dbk\limitpostsperuser\event;
...
class listener implements EventSubscriberInterface
{
	...
	*/
	static public function getSubscribedEvents()
	{
		...
		return array(
			'core.modify_posting_auth'	=> 'check_limit_posts_per_user',
	...
	*/
	...
	public function load_language($event)
	{
		...
		if (( ($data['lang_set']) && ($data['lang_set'][0]=='posting')) &&
			...
			( $this->config['lppu_enable'] ) &&					
		 	(	($this->config['lppu_per_day_check']) || 
		     	($this->config['lppu_per_week_check']) || 
				($this->config['lppu_per_month_check']) ) )
		{			
			$this->user->add_lang_ext('dbk/limitpostsperuser', 'common');
		}	
		return;
	}
	...
	*/
	public function check_limit_rules_to_apply($event, $forum_id, $settings)
	{
		...
		if (trim($settings)>'') // empty settings never apply
		{
			...
			{ 
				...
				foreach ($rule_forum_parts as $rule_forum_part)
				{
					...
					$max_posts_for_rule = intval(trim($rule_parts[1])); // 10 ...or [10] ?? does intval() help here??
					...
					foreach ($rule_forums as $rule_forum) 
					{
						...
						if ($rule_forum == '*') 
						{
							$max_posts_for_all = $max_posts_for_rule;
						...
						{  
							...
							$out['max_posts'] = $max_posts_for_rule; 
		...
		{
			$out['do_check'] = true;						
			$out['max_posts'] = $max_posts_for_all; 		
	...
	/**
	* count the posts a user started in (a combination of) forum_id(s) since date $since
	*
	* @param object $event The event object
	* @return number of posted posts
	* @access public
	*/
	
	public function count_posts_by_user_since($event, $forum_ids_to_check, $user_id, $since)
	{
		$sql = "SELECT count(distinct post_id) as post_count FROM ". POSTS_TABLE. 
				" WHERE forum_id IN (".$forum_ids_to_check.")".
				" AND `post_poster` =" .$user_id.
				" AND `post_time` > " .$since; //.";";
		...
		if ( !($result) )
		{
			trigger_error('Could not obtain posts this month count for this forum. sql='.$sql);
		}
	...
	*/
    ...
    public function do_timeframed_check($event, $forum_id, $user_id, $time_mode, $settings, $since, &$results)
	{
		...
		if ($check_ary['do_check'])
		{
			...
			$max_posts = intval($check_ary['max_posts']);
	  
			$post_count = intval( $this->count_posts_by_user_since($event, $forum_ids_to_check, $user_id, $since) );
	  
			if ($post_count >= $max_posts) 
			{				
				$results['time_mode'] = $time_mode;
				$results['limit_hit'] = $max_posts;
	...
	/**
	* Check posts per user limit (main event handler function)
	*
	* @param object $event The event object
	* @return null
	* @access public
	*/
	public function check_limit_posts_per_user($event)
	{
		// it's all about creating new posts here - if the "mode" is anything else than "post",
		...
		if ( ( !($event['mode']=='post') ) ||				
			 ...
			 ( !$this->config['lppu_enable'] )) // is checking globally disabled? - if so: nothing to do, just return.
		{
			return;
		}					
		...
		if (   (!$this->config['lppu_per_day_check']) 
		    && (!$this->config['lppu_per_week_check']) 
			&& (!$this->config['lppu_per_month_check']) )
		{
			// this code "smells" ;) 
		...
		$included_group_ids = Trim($this->config['lppu_included_group_ids']);
		...
		$excluded_rank_ids = Trim($this->config['lppu_excluded_rank_ids']);
		...
		// is checking new posts per DAY enabled? then do daily checks.
		
		if ($allow_new_post && ($this->config['lppu_per_day_check'])) 
		{			
			$allow_new_post = $this->do_timeframed_check(
				$event, 
				$forum_id, $user_id,
				'day',
				$this->config['lppu_per_day_settings'],
				strtotime('today midnight'),
				$results); // var param
		}
		
		// is checking new posts per WEEK enabled? then do weekly checks.
		
		if ($allow_new_post && ($this->config['lppu_per_week_check'])) 
		{
			$allow_new_post = $this->do_timeframed_check(
				$event, 
				$forum_id, $user_id,
				'week',
				$this->config['lppu_per_week_settings'],
			    strtotime('Last Monday', time()),
				$results); // var param
		}
		
		// is checking new posts per MONTH enabled? then do monthly checks.
		
		if ($allow_new_post && ($this->config['lppu_per_month_check'])) 
		{
			$allow_new_post = $this->do_timeframed_check(
				$event, 
				$forum_id, $user_id,
				'month',
				$this->config['lppu_per_month_settings'],
		...
		switch ($time_mode) {
			case 'day';
				$message = $this->user->lang['LPPU_ERROR_DAY'];
				break;
			case 'week';
				$message = $this->user->lang['LPPU_ERROR_WEEK'];
				break;
			case 'month';
				$message = $this->user->lang['LPPU_ERROR_MONTH'];
				break;
		}

		$message = sprintf($message,$limit_hit);
		
		$back_link = $this->user->lang['LPPU_ERROR_BACK_TO_FORUM'];		
